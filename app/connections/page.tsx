...
export default function ConnectionsPage() {
   ...
   return (
      <div className="min-h-screen pt-24 pb-40 px-4 md:px-8 max-w-7xl mx-auto flex flex-col gap-12 font-mono">
            ...
            <div className="w-full lg:w-1/3 flex flex-col gap-6 shrink-0">
               ...
               <div className="bg-panel/20 border border-white/5 rounded-lg overflow-hidden">
                  <div className="bg-white/5 px-4 py-2 border-b border-white/10 flex justify-between items-center">
                     ...
                     <span className="text-[10px] text-neon-gold">Nodes: {mockRoster.length}</span>
                  </div>
                  <div className="divide-y divide-white/5">
                     {mockRoster.map(op => (
                        ...
                        >
                           <div className="w-8 h-8 rounded-full bg-black/40 border border-white/10 flex items-center justify-center text-neon-gold shrink-0">⌬</div>
                           <div className="flex flex-col min-w-0">
                              <span className="text-xs text-white font-bold uppercase truncate">{op.name}</span>
                              <span className="text-[9px] text-white/30 uppercase truncate">{op.class} // {op.sign}</span>
                           </div>
                        </button>
                     ))}
                  </div>
               ...
               <div className="bg-panel/40 border border-neon-gold/30 rounded-lg p-6 backdrop-blur-md">
            ...
            <div className="w-full lg:w-2/3 lg:flex-1">
               <AnimatePresence mode="wait">
                  {selectedOperator ? (
                     ...
                     >
                        {/* Analysis Header */}
                        <div className="bg-black/40 p-6 rounded-lg border border-neon-gold/40 shadow-[0_0_20px_rgba(255,215,0,0.1)] w-full">
                           <h2 className="text-xl md:text-2xl font-bold text-white uppercase tracking-widest mb-1">Aether Energy Analysis</h2>
                           <p className="text-[10px] md:text-xs text-neon-gold uppercase tracking-[0.2em]">Synastry: You + {selectedOperator.name}</p>
                        </div>

                        {/* Resonance Card */}
                        <div className="bg-white/5 p-6 rounded-lg border border-white/10 w-full">
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 w-full">
                              ...
                              <ul className="space-y-4">
                                 {["Astral Sandbox collaboration", "Kinetic calibration exercises", "Shared sensory grounding sessions"].map((act, i) => (
                                    <li key={i} className="flex items-start gap-3">
                                       ...
                                       <span className="text-xs text-white/70 uppercase font-mono leading-relaxed">{act}</span>
                                    </li>
                                 ))}
                              </ul>
                           </div>
                           ...
                           <div className="bg-white/5 p-6 rounded-lg border border-white/10 h-full">
                              <h3 className="text-xs font-bold text-neon-cyan uppercase tracking-widest mb-4">Discussion Vectors</h3>
                              <ul className="space-y-4">
                                 {["Bridging elemental gaps via sign logic", "Multi-dimensional career trajectories", "Optimizing collective epicyclic output"].map((vec, i) => (
                                    <li key={i} className="flex items-start gap-3">
                                       ...
                                       <span className="text-xs text-white/70 uppercase font-mono leading-relaxed">{vec}</span>
                                    </li>
                                 ))}
                              </ul>
                           </div>
                        ...
                        <div className="bg-red-950/20 border border-red-500/30 p-4 rounded text-center w-full mt-6">
                     ...
                     <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        className="w-full h-full min-h-[400px] border border-dashed border-white/10 rounded-lg flex flex-col items-center justify-center p-12 text-center"
                     >
