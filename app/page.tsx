...
type AppPhase = "SPLASH" | "LOGIN" | "INTAKE" | "DIAGNOSTIC" | "SOVEREIGN";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Auto-bypass SPLASH/LOGIN to INTAKE if authenticated but NOT registered
    if (status === 'authenticated' && (phase === "SPLASH" || phase === "LOGIN") && !isRegistered) {
      setPhase("INTAKE");
      return;
    }
    ...
    <main className="min-h-screen bg-background text-foreground font-mono overflow-hidden pb-40">
      <AnimatePresence mode="wait">
        ...
        {phase === "SPLASH" && (
          ...
          >
            <button 
               onClick={() => setPhase(status === 'authenticated' ? "INTAKE" : "LOGIN")}
        ...
        {phase === "INTAKE" && (status === 'authenticated' || isGuest) && (
          <motion.div
            key="intake"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
