               ...
               >
                  ...
                  {avatar ? (
                     <>
                        <img
                           src={avatar}
                           alt="Operator Insignia"
                           className={`absolute inset-0 w-full h-full object-cover opacity-80 mix-blend-screen transition-all duration-700 ${isGenerating ? 'grayscale brightness-50 blur-sm scale-110' : 'group-hover:scale-110'}`}
                        />
                        {isGenerating && (
                        ...
                        {!avatar ? (
                           ...
                           <div className="flex flex-col items-center gap-2 w-full">
                              <button
                                 ...
                                 className={`w-full py-2 bg-transparent hover:bg-white/5 border border-white/20 rounded text-foreground/70 hover:text-white font-mono text-[10px] uppercase tracking-widest transition-all flex justify-center items-center gap-2 ${isGenerating ? "opacity-50 cursor-not-allowed" : ""}`}
                              >
                              ...
                              <div className="flex w-full gap-2 mt-2">
                                 ...
                                 {userProfileImage !== avatar ? (
                                    <button
                                       ...
                                       className="flex-1 py-2 bg-neon-cyan/10 hover:bg-neon-cyan/20 border border-neon-cyan/30 rounded text-cyan-200 font-mono text-[10px] uppercase tracking-widest transition-all"
                                    >
                                       [ Set Profile ]
                                    </button>
                                 ) : (
                                    <div className="flex-1 py-2 flex justify-center items-center border border-emerald-500/20 bg-emerald-500/10 rounded">
                                       <span className="text-[10px] text-emerald-400 font-mono tracking-widest uppercase flex items-center">
                                 ...
                                 <a
                                    href={avatar}
                                    download={`Aether-Insignia-${operatorClass?.replace(/\s+/g, '-') || 'Operator'}.png`}
                                    className="flex-1 py-2 flex justify-center items-center bg-neon-purple/10 hover:bg-neon-purple/20 border border-neon-purple/30 rounded text-neon-purple font-mono text-[10px] uppercase tracking-widest transition-all"
                                 >
                                    [ Download ]
                                 </a>
                              </div>
